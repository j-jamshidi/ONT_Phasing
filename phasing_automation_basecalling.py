...
def compare_variants(dummy_vcf: str, variant_calling_vcf: str) -> List[Tuple[str, int, str, str]]:
    ...
    print(f"\nComparing variants:")
    print(f"Dummy VCF: {dummy_vcf}")
    print(f"Variant Calling VCF: {variant_calling_vcf}")
    ...
    print(f"Dummy VCF variants found: {len(dummy_variants)}")
    ...
    try:
        ...
        for record in vcf.fetch():
            # Only add if alternate allele exists
            if record.alts and record.alts[0]:
                variant_calling_variants.append((record.chrom, record.pos, record.ref, record.alts[0]))
        
        print(f"Variant calling VCF variants found: {len(variant_calling_variants)}")
...
def write_variant_comparison_results(dummy_vcf: str, variant_calling_vcf: str, output_file: str):
    ...
    try:
        ...
        with open(output_file, 'a') as f:
            ...
            f.write(f"Dummy VCF: {dummy_vcf}\n")
            f.write(f"Variant Calling VCF: {variant_calling_vcf}\n\n")
            
            f.write("Dummy VCF Variants:\n")
            # Read dummy VCF variants
            ...
            f.write(f"\nSummary:\n")
            f.write(f"Total variants in dummy VCF: {len(matched_variants)}\n")
            f.write(f"Variants found: {found_count}\n")
            f.write(f"Variants not found: {not_found_count}\n")
        
        print("Variant comparison results written successfully.")
...
def quality_control(input_bam, vcf_file, output_bam):
    ...
    if high_quality_reads > 50:
        print("QC PASSED for the quality (Q>20) and depth (>50) of sequencing")
    else:
...
def main():
    ...
    barcode = os.path.basename(os.path.dirname(bam_file))
    ...
    with open(output_txt, 'a') as output_file:
        ...
        try:
            # Update paths to work with current directory structure
            current_dir = os.getcwd()
            barcode_dir = os.path.join(current_dir, barcode)
            
            # Update the path to the dummy VCF file
            dummy_vcf = os.path.join(barcode_dir, f"{episode}.vcf")
            
            # Find the variant calling VCF 
            variant_calling_vcf = os.path.join(current_dir, "variant_calling_output", f"{episode}.wf_snp.vcf.gz")
            
            # Check if dummy VCF exists
            if not os.path.exists(dummy_vcf):
                print(f"Warning: Dummy VCF file not found at {dummy_vcf}")
                print("Files in current directory:")
                print(os.listdir(current_dir))
                print(f"Files in barcode directory {barcode_dir}:")
                print(os.listdir(barcode_dir))
            
            # Check if variant calling VCF exists and perform comparison if both files are present
